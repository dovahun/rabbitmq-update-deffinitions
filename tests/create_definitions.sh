...
renderHelmChart() {
  helm template \
    ...
    -f $PATH_TO_RABBITMQ_DEFINITIONS_DIR/globalParameters.yaml \
...
makeDefinitionFile() {
  yq e '.. |  select(has("definitions.json")) | ."definitions.json"' $render_manifest_file > $definitions_file_in_base64
